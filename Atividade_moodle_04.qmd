...
Using arithmetic returns from the adjusted prices, what is the value of beta for stock WY?
...
lm(return_WY ~ return_GSPC, data = df_base)
...
Based on file EX_SP500-stocks.rds, and using price and return data only for asset CMI, estimate the following lagged model:
...
lm(return_CMI ~ lag_mercado, data = df_base)

```

## Questão 4

-1 para remover intercepto do modelo

Using the data from file EX_SP500-stocks.rds, estimate the following model for stock ULTA:


R_t = \beta _1 R_{M,t} + \sum ^{5} _{i=1} \theta _i WeekDummy _{i,t} +  \epsilon _t

Where WeekDummy _{i,t} is a dummy variable that takes value 1 if the day of the week is i (monday to friday). What is the estimated value of parameter \theta _1?

```{r}

df_base <- df_sp500 |> 
  filter(ticker %in% c('^GSPC', 'ULTA')) |> 
  mutate(ticker = str_replace(ticker, '\\^', '')) |> 
  select(ref_date, ticker, ret_adjusted_prices)
...
Using file EX_SP500-stocks.rds, consider the estimation of the beta model for stock ULTA. Using CRAN package car, perform a linear hypothesis test for \alpha = 0 and \beta = 0.7. What is the p-value of the test?
...
modelo1 <- lm(return_ULTA ~ return_GSPC, data = df_base)
...
lh <- linearHypothesis(modelo1, c("(Intercept) = 0", "return_GSPC   = 0.7"))
lh
...
After loading the data from file EX_SP500-stocks.rds, estimate the following beta (market) model for stock DGX:
...
modelo1 <- lm(return_DGX ~ return_GSPC, data = df_base)
...
Load the data from file EX_SP500-stocks.rds and keep all rows for the following tickers: NTRS, CINF, TRGP, LEN, DGX, AFL, HES, SPG, ^GSPC, NLOK, V, D, CMI, GOOGL, ZBH, BA, TGT. Using package lmtest, perform the Durbin Watson test for the market model of each stock. How many of the 16 stocks reject the null hypothesis of serial correlation at 5%?

```{r}

tickers <- c('NTRS', 'CINF', 'TRGP', 'LEN', 'DGX', 'AFL', 
             'HES', 'SPG', '^GSPC', 'NLOK', 'V', 'D', 'CMI', 
             'GOOGL', 'ZBH', 'BA', 'TGT')
...
Again using data from file EX_SP500-stocks.rds, filter the dataset by removing the following tickers: PEP, AFL, TGT, NKE, SO, LEN, V, FCX, WY, ULTA, HES. With the resulting dataframe, calculate the beta (market model) coefficient for all available remaining stocks. What is the average value of Beta?

```{r}

remover_tickers <- c('PEP', 'AFL', 'TGT', 'NKE', 'SO', 'LEN', 
                     'V', 'FCX', 'WY', 'ULTA', 'HES')
...
tickers <- c('LEN', 'VRSN', 'FCX', 'SBUX', 'HES', 'PEP', 
                     'DE', 'DGX', 'ORLY', 'TRGP', 'V', 'KEY', 'GOOGL', 
                     'NDAQ', 'D', 'JKHY', 'TWTR', 'BA', 'NTRS', '^GSPC')
...
where P(R _t > 0) is the probability that the return of a particular stock is higher than 0 and g() is the probit function. Estimate this GLM model using R and, looking the result for stock HES, what is the value of \beta?
...
# returno maior que 0
df_base <- df_base |> 
  mutate(retorno_positivo = ifelse(return_HES > 0, 1, 0))

lm(return_HES ~ return_GSPC, df_base)  
...
Using set.seed = 13 and function arima.sim, simulate 155 observations of the following ARMA model:

y_t = 0.4 y_{t-1} + -0.09 \epsilon _{t-1} + \epsilon _{t}

\epsilon _{t} \sim N(0, var = 0.25)
...
Again, using set.seed = 81 and function arima.sim, simulate 384 observations of the following ARMA model:

y_t = 0.4 y_{t-1} + -0.11 \epsilon _{t-1} + \epsilon _{t}

\epsilon _{t} \sim N(0, var = 1.69)
...
Load the data from file EX_SP500-stocks.rds and, based on the information for stock AFL, estimate an ARIMA(2, 0, 2) model for the returns of adjusted prices. What is the sum of all AR parameters?
...
Again, load the data from file EX_SP500-stocks.rds and, based on the information for stock TWTR, estimate an ARIMA(2, 0, 1) model for the returns of adjusted prices. Based on the estimated model, what is the forecasted return for t+1
...
Use function forecast::auto.arima to estimate the best model for the adjusted returns of stock TWTR from file EX_SP500-stocks.rds. Using the AIC criteria (see input ic in auto.arima). What is the optimal lag for the AR parameter?
...
tickers <- c('BBDC4.SA', 'ECOR3.SA', 'LREN3.SA', 'BRML3.SA', 
             'TOTS3.SA', 'AZUL4.SA', 'SANB11.SA')
...
Again, load the data from file EX_B3-stocks.rds and keep rows for the following assets: YDUQ3.SA, ENGI11.SA, BBDC4.SA, SANB11.SA, RAIL3.SA, BRML3.SA, LREN3.SA.Now, use function forecast::auto.arima to estimate the best model using a maximum order (input max.order) of 6. Based on the arima models perform, for all available assets, a t+1 forecast for the conditional mean. Which stock has the highest value of forecasted return at time t+1?

```{r}
tickers <- c('YDUQ3.SA', 'ENGI11.SA', 'BBDC4.SA', 'SANB11.SA', 
             'RAIL3.SA', 'BRML3.SA', 'LREN3.SA')
...
\end{aligned}
Using set.seed = 54, do 182 simulations of the process for 705 time periods. Looking at the simulated value at time 676 for all simulations, what is the maximum found value across all simulations? Tip: Make sure set.seed is used after calling garchSpec(). Somehow this function, garchSpec(), is using something random and messes up the queue needed for solving the exercise.
...
Using package fGarch, estimate a ARMA(1,1)-GARCH(1,1) model for the returns of stock NLOK, available in file EX_SP500-stocks.rds. What is the value of the ARCH coefficient (alpha1)?
...
Using data available in file EX_SP500-stocks.rds for stock ^GSPC, use your newly created function with max.lag = 2. What is the best Arma-Garch formula for the dataset?

```{r}

df_ticker <- filter(df_sp500, ticker == "^GSPC")
...
Using data available in file EX_SP500-stocks.rds for stock BA, use your newly created function with max.lag = 2 and criteria = HQIC. What is the (new) best formula for the dataset?

```{r}
df_ticker <- filter(df_sp500, ticker == "BA")
...
Nome	Valor
Maturidade (dias)	50
Preço de exercicio	18
Assumindo uma taxa de juros para o período inteiro de 50 dias como 15,00%, use o modelo RW anterior para calcular o preço justo desta opção de compra com 1000 simulações. (Utilize set.seed(92) antes da rodagem do código).
...
Utilizando set.seed(57) e nSim =1000, considere uma opção de venda do tipo européia com as seguintes características:

Nome	Valor
Maturidade (dias)	50
Preço de exercicio	15
